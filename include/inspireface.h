...
/*
 * @brief Struct for holding the properties of the tokens
 */
typedef struct {
    HFFaceBasicToken *ptr;
    HInt32 len;
} HFFaceBasicTokenSlice;

/*
 * @brief get the tokens from a HFMultipleFaceData struct
 */
HYPER_CAPI_EXPORT extern HFFaceBasicTokenSlice HFGetTokens(HFMultipleFaceData *data);
...
HYPER_CAPI_EXPORT extern HResult HFFaceFeatureWithRefExtractTo(HFSession session, HFImageStream streamHandle, HFFaceBasicToken *singleFace,
                                                        HFFaceFeature *feature);
